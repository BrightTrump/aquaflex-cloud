            ...
            @foreach ($products as $product)
                <div
                    ...
                    <div class="h-[17rem] w-full rounded-xl">
                        <img src="{{ $product->thumbnail }}" class="w-full h-full object-cover rounded-xl" />
                    </div>
                    <div class="p-8 text-[20px] font-semibold flex flex-col gap-6 justify-between h-full">

                        <div class="h-[13rem] flex flex-col justify-between">
                            ...
                            <div class="flex gap-6">
                                <div class="w-1/2 h-[5.5rem] bg-sky-50 border border-sky-300 rounded-[1rem] p-4">
                                    <h1 class="font=semibold text-[13px]">{{ $product->size }}{{ $product->unit }} x
                                        {{ $product->quantity }} Bottle</h1>
                                    <span class="text-[10px] block">{{ $product->size }}{{ $product->unit }} x
                                        {{ $product->quantity }}Bottle</span>
                                </div>
                                <div
                                    class="w-1/2 h-[5.5rem] bg-slate-50 border border-slate-300 rounded-[1rem] flex items-center justify-center">
                                    ₦{{ $product->price }}
                                </div>
                            </div>
                        </div>
                        <div>
                            <a href="{{ route('add_to_cart', $product->id) }}"
                                ...
                                <span class="absolute left-6"> <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
